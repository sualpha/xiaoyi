...
const cfu = {
	...
	"formatter": {
		...
		"customPieData": function(val, index, series, opts) {
			if (index !== undefined) {
				return series[index].name + ' ' + (val._proportion_ * 100).toFixed(2) + '%';
			}
		},
		"customXAxisLabelForMonth": function(val, index, series, opts) {
			if (index % 5 === 0) {
				return val;
			} else {
				return '';
		...
		"customXAxisLabelForYear": function(val, index, series, opts) {
			if (index % 2 === 0) {
				return val;
			} else {
				return '';
